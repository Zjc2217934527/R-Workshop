...
#is there a correlation between the survival of passengers and the Pclass they were in?

  ggplot(passengers, aes(x=as.factor(Survived), fill=as.factor(Pclass))) +
  geom_bar()+
  labs(x="Survival of Passengers: 0=Dead, 1=Alive", y="Number of Passengers", fill="Passenger Class Value", title="Correltion between Passenger Class Value and Survival of Passengers")
  
# group the passengers by their group and find the total amount of passengers survived over the total amount of passengers
passengers %>% 
  group_by(Pclass) %>% 
  summarize("Survived Passengers"=sum(Survived), "Total Passengers"=n())

  #chisq.test(x=c(136,87,119))
  chisq.test(x=matrix(c(80,136,97,87,372,119),ncol=2))
```
