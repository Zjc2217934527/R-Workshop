...
*Note: Lines within r chunks starting with* `##` *need to be completed or edited*
...
We now have a function called `ExponentialGrowth()` that describes the exponential growth model and the 3 required arguments for the `lsoda()` function. Let's simulate the population and save it as an object called `sim1` 

```{r exp population simulation}
##sim1 <- lsoda(y = y0, times = , func = , parms = ) #complete the arguments
sim1 <- lsoda(y = y0, times = t.vector, func = ExponentialGrowth, parms = parameters) #complete the arguments
```
...
##parameters2 <- c() # create a new vector for the params argument
parameters2 <- c(r=0.5)
##sim2 <- lsoda(y = y0, ) # complete the arguments 
...
##ggplot(data = , aes(x = , y = )) +  geom_point() # complete the code with the appropriate data, and x-y aesthetics 
ggplot(data = sim1, aes(x = time, y = `1`)) + 
  geom_point()#
...
# start with the original sim1 plot (sim1 plot code chunk)
 # we can add points and lines from sim2 by calling the sim2 data, and defining the aesthetic
  ...
  ##geom_line(data = , aes( ))
ggplot(data = sim1, aes(x = time, y = `1`)) + 
geom_point(data =sim2, aes(time, `1`))+
  geom_line(data =sim2, aes(time, `1`))
...
p <- ggplot(data = sim1, aes(x = time, y = `1`)) + 
      geom_point() +
      geom_line()
...
p + geom_point(data = sim2, aes(x = time,y = `1`)) + 
    geom_line(data = sim2, aes(x = time,y = `1`), color = 'pink')
...
#Complete the function for:
   ...
   dS <- -beta*S*I ## expression for dS
   dI <- beta*S*I-gamma*I ## expression for dI
   dR <- gamma*I ## expression for dR
...
##longSIRsim <-  %>% pivot_longer(cols = S:R, names_to = "state", values_to = "value")
...
SIRsim %>% 
  pivot_longer(cols = S:R, names_to = "state", values_to = "value") %>%  # the pivoted data is fed directly into ggplot without creating a secondary object. 
